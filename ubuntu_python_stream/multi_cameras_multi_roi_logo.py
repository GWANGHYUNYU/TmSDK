#################################################################
# File: multi_cameras_multi_roi_logo.py
#
# 실행:
#   python3 multi_cameras_multi_roi_logo.py
#
# 기능:
#   - 열화상 카메라 N대 동시 스트리밍 (런타임에 추가 가능)
#   - 각 열화상 카메라별 복수 ROI 설정 (Spot, Line, Rect, Ellipse)
#   - ROI 타입 선택 후 드래그로 추가, 개별 삭제 또는 전체 초기화
#   - 각 ROI별 온도(Min/Avg/Max) 개별 측정 및 CSV 저장
#   - 각 열화상 카메라별 또는 전체 일괄 녹화
#   - AVI 자동 분할 (1.4 GB 또는 1시간 초과 시)
#   - 열화상 카메라 연결 끊김 시 자동 재연결 + 녹화 자동 재개
#   - CSV 주기적 flush (데이터 유실 방지)
#   - 로그 파일 자동 기록 (logs/tmsdk.log, 일별 로테이션 60일 보관)
#   - 예약 녹화: 시작/종료 시간 지정, 반복 간격 설정 가능
#   - ★ 프리뷰 및 녹화 영상 오른쪽 하단에 로고 오버레이
#
# 요구사항:
...
# ── 로고 설정 ────────────────────────────────────────────────
# resource/logo1.png 경로 (실행 파일 기준 상대 경로)
LOGO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "..", "resource", "logo1.png")
LOGO_HEIGHT   = 40          # 프리뷰에 표시할 로고 높이 (px)
LOGO_MARGIN   = 8           # 오른쪽/하단 여백 (px)
LOGO_OPACITY  = 0.85        # 투명도 (0.0 ~ 1.0)
...
def _load_logo_pixmap() -> QPixmap:
    """로고 PNG를 읽어 LOGO_HEIGHT 높이에 맞게 축소한 QPixmap 반환."""
    if not os.path.isfile(LOGO_PATH):
        logger.warning(f"[로고] 파일을 찾을 수 없습니다: {LOGO_PATH}")
        return QPixmap()
    pix = QPixmap(LOGO_PATH)
    if pix.isNull():
        logger.warning(f"[로고] 이미지 로드 실패: {LOGO_PATH}")
        return QPixmap()
    pix = pix.scaledToHeight(LOGO_HEIGHT, Qt.SmoothTransformation)
    logger.info(f"[로고] 로드 완료: {pix.width()}x{pix.height()} px")
    return pix


def _load_logo_cv() -> np.ndarray | None:
    """로고 PNG를 읽어 LOGO_HEIGHT 높이에 맞게 축소한 BGR numpy 배열 반환.
    알파 채널이 있으면 4채널(BGRA)로 반환."""
    if not os.path.isfile(LOGO_PATH):
        return None
    ...
    scale = LOGO_HEIGHT / h
    new_w = int(w * scale)
    logo = cv2.resize(logo, (new_w, LOGO_HEIGHT), interpolation=cv2.INTER_AREA)
    return logo


# 전역에서 한 번만 로드
_LOGO_PIXMAP: QPixmap | None = None   # QApplication 생성 후 초기화
_LOGO_CV: np.ndarray | None = _load_logo_cv()


def _get_logo_pixmap() -> QPixmap:
    """지연 초기화: QApplication이 있어야 QPixmap 생성 가능."""
    global _LOGO_PIXMAP
...
def _overlay_logo_cv(bgr: np.ndarray) -> np.ndarray:
    """OpenCV BGR 프레임의 오른쪽 하단에 로고를 합성한다."""
    if _LOGO_CV is None:
        return bgr
    fh, fw = bgr.shape[:2]
    lh, lw = _LOGO_CV.shape[:2]

    # 프레임보다 로고가 크면 스킵
    if lw + LOGO_MARGIN > fw or lh + LOGO_MARGIN > fh:
        return bgr

    # 오른쪽 하단 좌표
    x1 = fw - lw - LOGO_MARGIN
    y1 = fh - lh - LOGO_MARGIN
    x2 = x1 + lw
    y2 = y1 + lh

    roi = bgr[y1:y2, x1:x2]

    if _LOGO_CV.shape[2] == 4:
        # BGRA → 알파 블렌딩
        alpha = (_LOGO_CV[:, :, 3] / 255.0) * LOGO_OPACITY
        alpha3 = np.stack([alpha] * 3, axis=-1)
        logo_bgr = _LOGO_CV[:, :, :3]
        blended = (logo_bgr * alpha3 + roi * (1 - alpha3)).astype(np.uint8)
    else:
        # BGR → 단순 블렌딩
        blended = cv2.addWeighted(_LOGO_CV, LOGO_OPACITY,
                                  roi, 1 - LOGO_OPACITY, 0)

    bgr[y1:y2, x1:x2] = blended
    return bgr
...
class PreviewLabel(QLabel):
    ...
    def set_frame(self, img: np.ndarray):
        ...
        # ★ 로고 오버레이 (오른쪽 하단)
        logo_pix = _get_logo_pixmap()
        if not logo_pix.isNull():
            lw, lh = logo_pix.width(), logo_pix.height()
            x = w - lw - LOGO_MARGIN
            y = h - lh - LOGO_MARGIN
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("TmSDK Multi Camera Multi-ROI Stream (Logo) 시작")
    logger.info("=" * 60)
