...
## 5-6. ★ 4차 촬영 지침 (2026-09-15 작성 · **2026-09-18 실행 완료**)

> ✅ **이 촬영으로 캘리브레이션이 끝났습니다.** 결과는
> [status.md 0-F](status.md), 확정값은 [`params/`](../params/README.md).
> 아래는 무엇을 왜 그렇게 찍었는지의 기록입니다.
...
### ① 시간 — 소등 전에 끝내야 합니다 ★ 가장 중요

**RGB 는 소등(18:01)과 함께 멈춥니다.** 실제로 마지막 RGB 파일
`17-58-01.mp4` 가 18:00:02 에 끝났습니다. 열화상은 17:57:35 에 시작해
19:12 까지 돌았고, **123건 중 122건이 소등 이후**였습니다.

> **16:30 시작 · 17:45 종료.** 소등 15분 전에 손을 떼십시오.
> 열화상만 더 찍는 것은 그 뒤에 해도 됩니다.

| | 3차 실제 | 4차 목표 |
|---|---|---|
| RGB 와 겹친 녹화 | **1건** | 30건 이상 |
| 촬영 시간대 | 17:57 ~ 19:12 | **16:30 ~ 17:45** |

### ② 판 재가열 — 5~6자세마다 ★

3차의 보드 대비는 **75분에 걸쳐 단조 감소**했습니다. 한 번 데우고 끝까지
간 것입니다.

| 시간대 | 17:57~18:12 | ~18:22 | ~18:52 | ~19:03 |
|---|---|---|---|---|
| 보드 대비 중앙 | 1.62 ℃ | 1.29 ℃ | 1.04 ℃ | **0.91 ℃** |

2차(08-28)는 중앙 **4.06 ℃** 였고 검출이 11/15 였습니다.
3차는 중앙 **1.24 ℃**, 55건 중 17건이 1 ℃ 미만이었습니다.

> **자세 5~6개마다 뒷면에서 재가열.** 목표 2 ℃ 이상, 되도록 4 ℃.
> 눈짐작 하지 마시고 ⑥ 의 즉시 판정기를 보십시오.

**눈으로 보이는 것과 열화상에 잡히는 것이 다릅니다.**
`output/shoot_guide/contrast_bands.png` 에 5.8 ℃ / 2.6 ℃ / 1.4 ℃ 가
실제로 어떻게 보이는지 나란히 붙여 두었습니다. 1.4 ℃ 줄은 사람 눈으로도
거의 무늬가 안 보입니다 — **그 상태로 찍고 계셨던 것입니다.**

### ③ LED 바 — 피하려고 애쓰지 마십시오

측정해 보면 **LED 바는 주범이 아닙니다.**

- 금속 칸끼리의 온도 산포(= 반사 얼룩의 크기) 중앙 **0.25 ℃**
- 2차에도 같은 LED 바가 있었고 반사 산포는 0.75~1.08 ℃ 로 **오히려 컸는데**
  검출은 잘 됐습니다. 대비가 4~5.8 ℃ 였기 때문입니다

즉 **반사 얼룩 ÷ 대비** 가 문제입니다. 대비 4 ℃ 면 얼룩은 6 % 잡음이고,
대비 1.3 ℃ 면 20 % 가 됩니다. ② 를 지키면 LED 바는 저절로 무해해집니다.

> **LED 를 끄는 것은 답이 아닙니다.** 끄면 RGB 가 못 찍습니다.
> 그리고 ① 때문에 어차피 소등 전에 끝내야 합니다.

그래도 얼룩이 거슬리면, **판을 좌우로 돌리지 말고 위아래로 기울이십시오.**
금속면이 거울이므로, 판 윗변을 카메라 쪽으로 눕히면 금속 칸이 **바닥**을
비춥니다 (LED 바가 아니라). 이것은 ④ 에서 어차피 필요한 동작입니다.

| 동작 | 효과 | 지침 |
|---|---|---|
| **면내회전** (화면 안에서 판을 굴림) | 보드만 작아짐. 얻는 것 없음 | **20° 이하로** |
| **기울임** (윗변을 앞뒤로 눕힘) | 초점거리·왜곡을 결정. 반사도 피함 | **6자세 이상 20° 넘게** |

3차에서 면내회전 20° 초과가 10자세 있었습니다. 그만큼 손해입니다.

### ④ 보드를 하나 더 만드십시오 — 칸 50 mm

칸 30 mm 보드는 **737 mm 까지**가 한계입니다
(= 147.4 × 30 ÷ 6 px, 검출기 하한 6 px). 3층 캐노피 1000 mm 를 못 찍습니다.

| 보드 | 칸 | 내부 코너 | 판 크기 | 쓸 거리 |
|---|---|---|---|---|
| 지금 것 | 30 mm | 7×4 | 240×150 mm | **330 ~ 650 mm** |
| **새로 만들 것** | **50 mm** | **5×4** | **300×250 mm** | **600 ~ 1100 mm** |

5×4 로 줄인 이유: 7×4 에 50 mm 를 쓰면 판이 400 mm 폭이 되어 가까운
...
```bash
python3 scripts/live_check.py <녹화폴더>              # 30 mm 보드
python3 scripts/live_check.py <녹화폴더> --cell 50 --pattern 5x4
```

새 녹화가 생길 때마다 몇 초 안에

```
[ 12] 171532  △ 주의   455mm  칸 9.7px  대비 1.6℃  기울임 24도  면내  6도  점유 28%
      → 대비 1.6℃ · 다음 자세 전에 재가열
  진행  330[####] 430[##..] 530[....]   기울임20도+ 4/6   합계 14
  남은 것  430~530mm 2개, 530~650mm 4개, 20도 이상 기울인 자세 2개
```

를 찍어 줍니다. **「남은 것」이 빌 때까지만 찍으면 됩니다** — 3차처럼
123건을 찍을 필요가 없습니다. 목표는 **보드당 20자세 안팎**입니다.

### 요약 — 4차 촬영 한 장

| | |
|---|---|
| **목표** | ✅ **달성** — ‖T‖ 53.3 mm 확정 (재분할 폭 1.34 mm) |
